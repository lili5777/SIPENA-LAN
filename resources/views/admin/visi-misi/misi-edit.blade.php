        ...
        <div class="row align-items-center">
            <div class="col">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h1 class="text-white mb-1">Edit Misi</h1>
                        <p class="text-white-50 mb-0">Perbarui misi perusahaan</p>
                    </div>
                </div>
            </div>
            <div class="col-auto">
            ...
            <div class="card border-0 shadow-lg">
                <div class="card-header bg-white py-3 border-0">
                    <h5 class="card-title mb-0 fw-semibold">
                        <i class="fas fa-bullseye me-2" style="color: #285496;"></i> Form Edit Misi
                    </h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('visi-misi.misi.update', $misi) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <div class="mb-3">
                            <label for="isi" class="form-label fw-semibold">Isi Misi <span
                                    class="text-danger">*</span></label>
                            <textarea class="form-control" id="isi" name="isi" rows="3"
                                required>{{ old('isi', $misi->isi) }}</textarea>
                            @error('isi')
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="ctt" class="form-label fw-semibold">Judul Singkat <span
                                        class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="ctt" name="ctt"
                                    value="{{ old('ctt', $misi->ctt) }}" maxlength="50" required>
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="icon" class="form-label fw-semibold">Icon <span
                                        class="text-danger">*</span></label>
                                <select class="form-select" id="icon" name="icon" required>
                                    @foreach($icons as $key => $label)
                                        <option value="{{ $key }}" {{ old('icon', $misi->icon) == $key ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
