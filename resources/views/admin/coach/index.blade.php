...
@section('title', 'Master Coach - Sistem Pelatihan')
...
@section('content')
        ...
        <div class="row align-items-center">
            <div class="col">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h1 class="text-white mb-1">Coach</h1>
                        <p class="text-white-50 mb-0">Kelola data coach pelatihan</p>
                    </div>
                </div>
            </div>
            <div class="col-auto">
                <div class="d-flex align-items-center gap-2">
                    <a href="{{ route('coach.create') }}" class="btn btn-light btn-hover-lift shadow-sm d-flex align-items-center">
                        <i class="fas fa-plus me-2"></i> Tambah Coach
                    </a>
            ...
            <div class="card border-0 shadow-sm stat-card">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div><h6 class="text-muted mb-1">Total Coach</h6><h3 class="mb-0 fw-bold text-primary">{{ $totalCoach }}</h3></div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card border-0 shadow-sm stat-card">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div><h6 class="text-muted mb-1">Aktif</h6><h3 class="mb-0 fw-bold text-success">{{ $aktifCoach }}</h3></div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card border-0 shadow-sm stat-card">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div><h6 class="text-muted mb-1">Non-Aktif</h6><h3 class="mb-0 fw-bold text-secondary">{{ $nonaktifCoach }}</h3></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            <form id="filterForm" method="GET" action="{{ route('coach.index') }}">
    ...
    <div class="card border-0 shadow-lg overflow-hidden">
        <div class="card-header bg-white py-3 border-0">
            <h5 class="card-title mb-0 fw-semibold">
                <i class="fas fa-list me-2" style="color: #285496;"></i> Daftar Coach
            </h5>
            <small class="text-muted">
                Menampilkan {{ $coach->firstItem() ?? 0 }} - {{ $coach->lastItem() ?? 0 }} dari {{ $coach->total() }} coach
            </small>
        </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        @forelse($coach as $item)
                            <tr class="coach-row">
                                <td class="ps-4 fw-semibold">
                                    {{ ($coach->currentPage() - 1) * $coach->perPage() + $loop->iteration }}
                                </td>
                                <td>
                                    <div class="d-flex align-items-center">
                                ...
                                <td class="text-center pe-4">
                                    <div class="btn-group" role="group">
                                        <a href="{{ route('coach.edit', $item->id) }}"
                                            class="btn btn-sm btn-outline-warning btn-action"
                                            data-bs-toggle="tooltip" title="Edit Coach">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <button type="button"
                                            class="btn btn-sm btn-outline-danger btn-action delete-coach"
                                            data-id="{{ $item->id }}"
                                            data-name="{{ $item->nama }}"
                                            data-kelompok="{{ $item->kelompok_count }}"
                                            data-bs-toggle="tooltip" title="Hapus Coach">
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center py-5">
                                    <i class="fas fa-user-tie fa-4x mb-3" style="color: #e9ecef;"></i>
                                    <h5 class="text-muted mb-2">
                                        @if(request()->hasAny(['status','search'])) Tidak ada hasil ditemukan
                                        @else Belum ada coach @endif
                                    </h5>
                                    @if(!request()->hasAny(['status','search']))
                                        <a href="{{ route('coach.create') }}" class="btn btn-primary mt-2">
                                            <i class="fas fa-plus me-1"></i> Tambah Coach
                                        </a>
                                    @endif
                                </td>
                            </tr>
        ...
        @if($coach->count() > 0)
            <div class="card-footer bg-white py-3 border-0">
                <div class="row align-items-center">
                    <div class="col-md-6">
                        <small class="text-muted">
                            Menampilkan {{ $coach->firstItem() }} - {{ $coach->lastItem() }} dari {{ $coach->total() }} coach
                        </small>
                    </div>
                    <div class="col-md-6">
                        <nav class="d-flex justify-content-md-end">
                            @if($coach->hasPages())
                                {{ $coach->links('pagination::bootstrap-5') }}
                            @else
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg">
                ...
                <div class="modal-body text-center px-4 pb-4">
                    ...
                    <h5 class="text-danger mb-4 fw-bold" id="deleteCoachName"></h5>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.delete-coach').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            document.getElementById('deleteCoachName').textContent = this.dataset.name;
            document.getElementById('deleteForm').action = `/coach/${this.dataset.id}`;
            ...
            if (kelompok > 0) {
                ...
                confirmMsg.innerHTML = `<i class="fas fa-info-circle me-1"></i> Tindakan ini tidak dapat dibatalkan. Semua data coach akan dihapus.`;
...
<style>
    ...
    .coach-row:hover { background-color:rgba(40,84,150,.03) !important; }
